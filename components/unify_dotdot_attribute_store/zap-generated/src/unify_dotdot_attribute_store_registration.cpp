...
sl_status_t unify_dotdot_attribute_store_registration_init()
{
...

  {
    ...
    if (storage_type == UNKNOWN_STORAGE_TYPE) {
      sl_log_warning(LOG_TAG,
                     "Unkown storage type for ZCL UnifyThermostat ThermostatMode, "
                     "type:  enum8 // uint8_t");
    }

    status |= attribute_store_register_type(
      DOTDOT_ATTRIBUTE_ID_UNIFY_THERMOSTAT_THERMOSTAT_MODE,
      "ZCL UnifyThermostat ThermostatMode",
      ATTRIBUTE_STORE_INVALID_ATTRIBUTE_TYPE,
      storage_type);
  }
...

  {
    ...
    if (storage_type == UNKNOWN_STORAGE_TYPE) {
      sl_log_warning(LOG_TAG,
                     "Unkown storage type for ZCL UnifyThermostat SupportedThermostatMode, "
                     "type:  map16 // uint16_t");
    }
